...
app.post('/api/chat', async (req, res) => {
    ...
    if (!apiKey) {
        return res.status(500).json({ error: 'Server configuration error: Gemini API key is missing. Please add it to your .env file.' });
    }
...
INSTRUCTIONS:
- Answer the student's question about this specific position based on the provided FEN and evaluation.
- Reference specific squares, files, and tactical themes.
- Explain the strategic reasoning (imbalances, plans, threats).
- Keep responses concise but highly educational (2-4 sentences usually).
- If they ask about a specific move, explain why it's good or bad based on the position's evaluation.
- Do NOT list all the pieces. Assume the student can see the board. Just give them the coaching advice.`;
