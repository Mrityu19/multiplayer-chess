...
function analyzeCurrentPosition() {
    ...
    engine.getBestMove(pos.fen, analysisSettings, (bestMoveRaw, evalData) => {
        ...
        // Use engine evaluation if available, otherwise fallback to material score
        const score = evalData ? evalData.score : evaluatePositionFromFen(pos.fen);
        pos.engineEval = score; // Cache it for future deltas
        ...
        if (analysisIndex === 0) {
            ...
            // Get previous score from cache if available, else calculate material
            let prevScore = 0;
            if (prevPos) {
                prevScore = prevPos.engineEval !== undefined ? prevPos.engineEval : evaluatePositionFromFen(prevFen);
            }
...
function buildPositionContext() {
    ...
    const tempGame = new Chess(pos.fen);
    const score = pos.engineEval !== undefined ? pos.engineEval : evaluatePositionFromFen(pos.fen);
    const evalString = score >= 0 ? '+' + score.toFixed(2) : score.toFixed(2);
...
ENGINE EVALUATION (Centipawns/Mate): ${evalString}
WHOSE TURN: ${tempGame.turn() === 'w' ? 'White' : 'Black'}
LEGAL MOVES: ${tempGame.moves().join(', ')}
